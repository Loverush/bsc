...
type Peer struct {
	...
	*p2p.Peer                         // The embedded P2P package peer
	...
	votespool     VotesPool          // Votes pool used by the broadcasters
	knownVotes    mapset.Set         // Set of vote hashes known to be known by this peer
	voteBroadcast chan []common.Hash // Channel used to queue votes propagation requests
...
// version.
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool, votespool VotesPool) *Peer {
	peer := &Peer{
		id:              p.ID().String(),
		Peer:            p,
		rw:              rw,
		...
		txpool:          txpool,
		voteBroadcast:   make(chan []common.Hash),
		votespool:       votespool,
...
// the peer's broadcast queue is full, the event is silently dropped.
func (p *Peer) AsyncSendVotes(hashes []common.Hash) {
	select {
	case p.voteBroadcast <- hashes:
		// Mark all the transactions as known, but ensure we don't overflow our limits
		for p.knownVotes.Cardinality() > max(0, maxKnownVotes-len(hashes)) {
			p.knownVotes.Pop()
		}
		for _, hash := range hashes {
			p.knownVotes.Add(hash)
	...
	case <-p.voteTerm:
		p.Log().Debug("Dropping vote propagation", "count", len(hashes))
